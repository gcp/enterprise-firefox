...
export const ConsoleClient = {
  ...
  async collectDevicePosture({ waitForAddons = false } = {}) {
    ...
    // The console identifies a device by its raw machine identifier (e.g. the
    // device serial); the source tier is reported alongside it so a change of
    // source can be told apart from a genuine hardware change.
