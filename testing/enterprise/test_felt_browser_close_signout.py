...
class BrowserCloseSignout(FeltTests):
    ...
    def test_browser_close_signout(self):
        ...
        # immediately and initiateShutdown() proceeds with signout.
        with self._child_driver.using_context("chrome"):
            self._child_driver.execute_script(
                "Services.prefs.setBoolPref('enterprise.promptOnSignout', false);"
            )

        # Trigger an application-level quit through the same path as
        # File > Quit. BrowserGlue._onQuitRequest intercepts the quit,
        # shows the signout prompt (bypassed by the pref above), then
        # cancels the quit and calls initiateShutdown() which signs out
        # via POST /sso/logout and force-quits.
