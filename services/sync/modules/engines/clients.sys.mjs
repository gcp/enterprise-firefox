...
ClientEngine.prototype = {
  ...
  async _checkMachineIdChanged() {
    ...
    if (storedMachineId && storedMachineId !== currentMachineId) {
      ...
      // Reset the FxA device registration first. If it fails (e.g. a transient
      // network error), leave the stored machine ID untouched and bail without
      // mutating Sync state, so the change is detected and retried on the next
      // sync rather than being silently lost.
      try {
        await this._resetFxADeviceRegistration();
      } catch (error) {
        this._log.warn(
          "Could not reset FxA device registration; will retry on next sync",
          error
        );
        return false;
      }
